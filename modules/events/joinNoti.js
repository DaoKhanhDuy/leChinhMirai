...
module.exports.run = async function({ api, event }) {
	...
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		...
		const fs = require("fs");
		return api.sendMessage("", event.threadID, () => api.sendMessage({body:`🔱🪂Kết nối thành công! \n\n🍓Sử dụng >help để biết toàn bộ lệnh có mặt trên bot này`, attachment: fs.createReadStream(__dirname + "/cache/joinbox/join.mp4")} ,threadID));
	}
	else {
		try {
