module.exports.config = {
    ...
    cooldowns: 10,
    envConfig: {
        "YOUTUBE_API": "AIzaSyA7QfBlpDC49lDIkA5s5Y_XV0WHwNbyUKg"
    }
};
...
module.exports.handleReply = async function ({
    ...
    try {
        const result = await axios.get(`https://api-7.chinhle4447.repl.co/sing?link=https://www.youtube.com/watch?v=${handleReply.link[event.body - 3]}`);
...
module.exports.run = async function ({
    ...
    if (args.join(" ").indexOf("https://") == 0) {
        ...
        const result = await axios.get(`https://api-7.chinhle4447.repl.co/sing?link=https://www.youtube.com/watch?v=${linkUrlSing}`);
        path1 = __dirname + `/cache/${event.senderID}.m4a`
        const getms = (await axios.get(`${result.data.link}`, {
            responseType: "arraybuffer"
        })).data;
