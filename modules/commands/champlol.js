...
var text1 = ag[0],
    text2 = ag[1];
if (args[0] == "list") {
axios.get(`https://api-12.chinhle4447.repl.co/lol/list`).then(res => {
...
const res = await axios.get(`https://api-12.chinhle4447.repl.co/lol?champion=${text2}`);
...
Chí mạng: ${crit}`, event.threadID, event.messageID)
      ...
      return api.sendMessage(`Sử dụng \nchamp list: để xem danh sách các nhân vật\nchamp info - tướng: để xem thông số`, event.threadID, event.messageID)} 
}
